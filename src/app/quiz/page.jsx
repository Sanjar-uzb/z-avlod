...
export default function QuizPage() {
  ...
  const method = useMemo(() => {
    if (!quiz) return null;
    return methods.find(m => m.id === quiz.methodId);
  }, [quiz]);
    ...
    <div className="container">
      <div className="card">
        ...
        <div className="row" style={{marginTop: 10}}>
          ...
          {method && <Link className="btn btnGhost" href={`/method?id=${method.id}`}>Metodga qaytish</Link>}
        ...
        {result && (
          <>
            ...
            <div className="row" style={{alignItems:"center"}}>
              ...
              <span className={`badge ${levelFromPercent(result.percent).cls}`}>Daraja: {result.level}</span>
            </div>
